...
def callback_query(call):
    ...
    bot.register_next_step_handler(
        call.message, input_start_time, booking_date
    )


def input_start_time(message, booking_date, success=True):
    print(booking_date)
    try:
        chat_id = message.chat.id
        ...
        if time_format_validation(value=start_time):
            ...
            bot.register_next_step_handler(
                message, input_end_time, start_time, booking_date)
        else:
            bot.register_next_step_handler(message, input_start_time, booking_date, success=False)
    except Exception as e:
        bot.reply_to(message, 'Упс, попробуйте начать по новой /start')


def input_end_time(message, start_time, booking_date, success=True):
    # try:
    chat_id = message.chat.id
    ...
    if time_format_validation(end_time):
        users = get_users(username=message.from_user.username)
        bot.send_message(
            chat_id=chat_id,
        ...
        bot.register_next_step_handler(
            message, input_end_time, start_time, booking_date, success=False
        )
    # except Exception as e:
    #     print(message)
    #     bot.reply_to(message, 'Упс, попробуйте начать по новой /start')
